...
func (f FileOutput) _writeToFile(logLine shared.InputData) error {
	...
	if fileList, err := filepath.Glob(f.Path + f.Filename); err == nil {
	...
	file, err := os.OpenFile(f.Path+f.Filename, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error getting file info for %s: %v", f.Path+f.Filename, err)
	}
	switch f.Rolling {
	case "daily":
		...
		if fileInfo.ModTime().Format("2006-01-02") != currentTime {
			...
			if err := os.Rename(f.Path+f.Filename, f.Path+newFileName); err != nil {
				return fmt.Errorf("error renaming file: %v", err)
			}
			file, err = os.OpenFile(f.Path+f.Filename, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
			if err != nil {
				return fmt.Errorf("error opening new file %s: %v", f.Path+f.Filename, err)
			}
		}
	case "hourly":
	...
	if logLine.Json != nil {
		if _, err := file.WriteString(fmt.Sprintf("%s %s: %v\n", logLine.Tag, logLine.FileName, logLine.Json)); err != nil {
			file.Close()
			return fmt.Errorf("error writing JSON to file %s: %v", f.Path+f.Filename, err)
		}
	} else if logLine.Raw != "" {
		if _, err := file.WriteString(fmt.Sprintf("%s %s: %s\n", logLine.Tag, logLine.FileName, logLine.Raw)); err != nil {
			file.Close()
			return fmt.Errorf("error writing raw log to file %s: %v", f.Path+f.Filename, err)
		}
	}
	if err := file.Close(); err != nil {
		return fmt.Errorf("error closing file %s: %v", f.Path+f.Filename, err)
	}
